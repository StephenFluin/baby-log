    ...
    template: `
        <div class="main">
            ...
            <tr *ngFor="let child of userData.connectedFamilies">
                <td><input type="text" [value]="child.name" (input)="update(child, $event)" /></td>
                <td><div class="icon-circle"><img
                    (click)="delete(child)"
            ...
            <form *ngIf="showNew" (submit)="new($event)">
                ...
                <tr *ngIf="!showCode"
                    ><td>Shared Child</td><td><input type="checkbox" (change)="showCode = true"
                /></tr>
                <tr *ngIf="showCode"><td>Shared Child Code</td><td> <input type="text" name="code" /></td>
                <td colspan="2" *ngIf="showCode">
                    Were you given a code from the bottom of an existing child? Feel free to leave
                    blank to create a new child.
        </td></tr>
        </table>
